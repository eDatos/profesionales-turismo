...
class EmpleoErrorCollection
{
	...
	public function __construct(array $errs = null)
	{
		...
		if ($errs != null)
		{
			foreach($errs as $err)
			{
				if (is_string($err))
				{
				    $a = new EmpleoError(NIVEL_ERROR, ERROR_GENERAL, $err);
				}
				else if (is_a($err, "EmpleoError"))
				{
					$a = $err;
				}
				else 
	...
	{
	    $this->errores[] = new EmpleoError(NIVEL_ERROR, $categoria, $err_msg, $err_detalles);
	}
	
	public function log_aviso($categoria, $err_msg, $err_detalles = null)
	{
	    $this->errores[] = new EmpleoError(NIVEL_ADVERTENCIA, $categoria, $err_msg, $err_detalles);
	}
