...
$page->set_page_path(array(PAGE_SELECT_ESTAB_MES_ANO_MULTIPLE));
		      ...
		      && (isset($_POST[ARG_NOMBRE_EST]) || isset($_POST[ARG_CODIGO_EST])))
{
	...
	else 
	{
		...
		$resultset = $e->searchByCodigoMultiple($query_estab_codigo);	
	}
}
...
$variables = array(
	...
	ARG_ESTIDS => $page->request_post_or_get(ARG_ESTIDS, NULL),
	'mes' => $page->request_post_or_get(ARG_MES, NULL),
	'ano' => $page->request_post_or_get(ARG_ANO, NULL)		
);

/// Render de la pagina
$page->render( "admin_select_estab_mes_ano_multiple_view.php", $variables );
