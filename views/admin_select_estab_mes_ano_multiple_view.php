...
$(document).ready( function() {
	...
	$("#procesarTodos").button().click(
		function() {
			if(confirm("Se dispone a operar con TODOS los cuestionarios encontrados.\n¿Está seguro?")==false)
        		return false;

			var formFecha=document.forms['select_estab_mes_ano'];
			this.form['<?= ARG_MES ?>'].value=formFecha['<?= ARG_MES ?>'].value;
			this.form['<?= ARG_ANO ?>'].value=formFecha['<?= ARG_ANO ?>'].value;
			this.form['<?= ARG_ESTIDS ?>'].value="";
			var cods=document.getElementsByName('codigos');
			this.form['<?= ARG_ESTIDS ?>'].value=cods[0].innerText;
			for(i=1;i<cods.length;i++)
				this.form['<?= ARG_ESTIDS ?>'].value+=","+cods[i].innerText;
		}
	);
  });
</script>
		...
		<div class="cuadro fondo_gris">
	      ...
	      <div class="subrayado"></div>
			<table width="auto" border="0">
              <tr>
                <td colspan="3" class="formhint">Introduzca los códigos de los establecimientos buscados, separados por comas, en la casilla siguiente:</td>
              </tr>            
              <tr>
                <td style="padding:10px;width: 200px;"><input name="codigo" type="text" value="<?= @$estab_codigo ?>" size="92"/></td>
			...
			<table>
              ...
              <tr>
                <td style="padding:10px;width: 200px;"><input name="nombre" type="text" value="<?= @$estab_nombre ?>" size="92" maxlength="92"/></td>
                  ...
                  <div id="tabla_resultados">
                        <table class="tablaresultado" width="100%">
                          ...
                          <?php foreach ($data as $row): ?>
                              <tr>
                                <td name="codigos"><?= $row['codigo'] ?></td>
                                <td><a name="enlace" href="<?= $this->build_url( $navToUrl, array(ARG_ESTIDS => $row['codigo']) ); ?>"><?= $row['nombre'] ?></a></td>
                                <td><?= $row['isla'] ?></td>
                   ...
                   <form name="operar_todos" action="<?= $this->build_url($navToUrl); ?>" method="post">
                   	<input type="hidden" id="<?= ARG_ESTIDS ?>" name="<?= ARG_ESTIDS ?>" value="" />
                   	<input type="hidden" id="<?= ARG_MES ?>" name="<?= ARG_MES ?>" value="" />
                   	<input type="hidden" id="<?= ARG_ANO ?>" name="<?= ARG_ANO ?>" value="" />
                   	<input class="search" style="float:right; margin: 8px 18px 0 0" id="procesarTodos" type="submit" value="Procesar todos">
                   </form>
        <?php else: ?>
